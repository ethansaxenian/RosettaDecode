...
def number2romannumeral(n):
    return ''.join([rnl[x][y] for x, y in zip(list(range(4)), str(n).zfill(4)) if n < 5000 and n > -1])
# Option 2
def number2romannumeral(n):
    return reduce(lambda x, y: x + y, list(map(lambda x, y: rnl[x][y], list(range(4)), str(n).zfill(4)))) if -1 < n < 5000 else None
